...
$fromDomain = "from.com"
$toDomain = "to.com"
$data_output = "$HOME\Desktop\data_output"
# 填写需要被移动的csv文件路径，多个csv文件用空格或者换行符分隔
$csvs = @"
...
$csvs | ForEach-Object { 
    ...
    if(! (Test-Path $_))
    {
        if($_.Trim())
        {
            Write-Host "$_ not exists"
        }
        return
    }
    $csv = Split-Path -Path $_ -Leaf # csv文件名
    Write-Host $csv
    $csvFullPath = "$data_output\$fromDomain\$csv"
    # 开始处理
    $csvfrom = "$data_output\$fromDomain\$csv"
    $csvdest = "$data_output\$toDomain\$csv"
...
### 将指定目录下的jpg,png筛选出来并处理

可以配合powershell完成此任务

```powershell
ls *jpg,*png|% FullName > $home/jpn.txt ;
python $pys\image_compressor.py   -R auto -p -F  -O -k -f webp  -r 1000 800  -I $home/jpn.txt
