...
class WC(API):
    ...
    def get_all_categories_from_file(self, csv_files):
        ...
        for file in csv_files:
            with open(file, mode="r", encoding="utf-8") as f:
                ...
                for row in reader:
                    categories.add(row[ProductDataFields.CATEGORIES.value])
        return categories
    ...
    def get_custom_attribute(self, product_data, name="mycustom"):
        ...
        # # 使用枚举后的代码
        name = product_data.get(ProductDataFields.ATTRIBUTE_NAME.value)
        value = product_data.get(ProductDataFields.ATTRIBUTE_VALUES.value, "")
        data = {
            "name": name,
            "slug": name,
            "visible": False,
    ...
    def get_product_data(self, product_data) -> dict:
        ...
        data = {}
        # 使用枚举后的代码
        sku = product_data[ProductDataFields.SKU.value]
        name = product_data[ProductDataFields.NAME.value]

        regular_price = product_data[ProductDataFields.REGULAR_PRICE.value]
        sale_price = product_data[ProductDataFields.SALE_PRICE.value]

        description = product_data[ProductDataFields.DESCRIPTION.value]

        tags_str = product_data[ProductDataFields.TAGS.value]
        tags = self.process_tags(tags_str)

        categories_str = product_data[ProductDataFields.CATEGORIES.value]
        categories = self.get_categories(categories_str)

        img_url = product_data[ProductDataFields.IMAGES.value]
    ...
    def batch_update_products(self, batch, task_id=-1):
        ...
        try:
            ...
            for idx, info in enumerate(res_creation_lst):
                sku = batch[idx][ProductDataFields.SKU.value]
                name = batch[idx][ProductDataFields.NAME.value]
    ...
    def upload_product(
        ...
        if upload_mode != "update_if_exist":
            # 上传失败时不尝试更新产品
            if self.existing_products_sku.get(
                product_data[ProductDataFields.SKU.value]
            ):
                # 如果已经存在该产品,则跳过
