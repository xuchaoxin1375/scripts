...
参数指定:
    支持从默认参数配置中读取参数，支持命令行参数覆盖配置参数。
    完善处理优先级和互斥关系(例如使用了-i的情况下,自动取消-n,尤其是从默认参数配置json中读取默认开关配置时.)
TODO:
改进日志:
    改造下载日志为便于针对性恢复下载进度的存档型日志(比如使用json记录每个图片的下载结果,尤其注意请求码中404这类没有重试意义的错误,在进度恢复或者重新下载尝试意义不大的图片应该在二次下载中跳过)
    下载任务的存档日志设计应该合理,除了日志文件内结构,还有日志存放位置(参考思路:将关键下载参数适当处理拼接到日志输出路径中,每次下载读取指定日志文件,并尝试恢复下载进度)

将curl_cffi使用session提高效率,代替默认的request.session

改进参数选项:
    支持(从命令行参数中)指定url间的分隔符列表
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--name-url-pairs",
        action="store_true",  # 开关式参数
        help=f'输入文件包含文件名和URL对，格式为"文件名 URL",限定{URL_SEPARATORS}中指定的符号分隔(如果可以的话,推荐>号作为url间分隔符,这比逗号,分号都更可靠)',
    )
    # 输出参数和控制
    parser.add_argument(
        "-o",
