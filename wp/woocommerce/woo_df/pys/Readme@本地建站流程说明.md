...
- 市面上的图片格式五花八门,虽然绝大多数都是jpg,png,以及少量webp,但是还有一些网站采用比较小众或者新兴的格式,例如,`heic`,`avif`,`tiff`.
- 尽管本仓库提供的图片下载和压缩方案对这些常见格式都做了支持(将格式转换为webp),但是个别情可能会转换失败(可能是因为图片数据不完整(破图),或者使用小众格式,但是图片本身仍然可被受支持的软件打开,尤其是浏览器,以及某些现代化图片查看器).这里推荐几个方案:
  1. 可靠性标杆:直接使用现代浏览器看图(浏览器本身就是一种优秀的看图软件,支持广泛的格式,正常网站的图片不太可能连浏览器都无法查看)
  2. 专门看图的高性能软件,支持的图片格式丰富:[PhoXoSee - The Fastest Image Viewer for Windows](https://phoxo.com/).
  3. 系统自带的照片+[Honeyview ](https://www.bandisoft.com/honeyview/),前者支持avif,但是性能不够好,但是后者是高性能不支持`avif`

> PhoXoSee在一致性上可能不如浏览器可靠,同一个图片如果更名后查看效果可能有差异.
>
> 由于压缩脚本默认将图片后缀更改为webp(即便格式转换失败,也会强制修改图片后缀),例如一个avif格式的图片文件扩展名被改为webp,如果把honeyView设置为默认查看webp查看器,导致方案3将打开图片时提示图片错误.
