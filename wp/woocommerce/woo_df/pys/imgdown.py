...
compressor_logger = logger.getChild("imgcompressor")
...
def add_log_handler():
    ...
    """
    # 在设置handler之前,需使用debug()打印日志
    debug(f"{__name__}:尝试添加控制台日志处理器(handler)")
...
def download_by_curl(
    ...
    if not use_remote_name:
        ...
        debug(f"使用远程文件名, 计算的文件名(basename供参考): {output_path}")
