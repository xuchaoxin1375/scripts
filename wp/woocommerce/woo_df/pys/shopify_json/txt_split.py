...
def generate_txt_excel_list(root_folder, output_excel='txt_list.xlsx', default_lines_per_file=100):
    txt_records = []

    for dirpath, _, filenames in os.walk(root_folder):
        for filename in filenames:
            if filename.lower().endswith('.txt'):
                full_path = os.path.normpath(os.path.abspath(os.path.join(dirpath, filename)))
...
def split_txt_by_line_count(input_file, lines_per_file):
    ...
    for i in range(total_parts):
        ...
        output_files.append({'input_file': input_file, 'output_file': output_file, 'part': i+1, 'lines': len(chunk)})
    ...
    if len(output_files) >= 2 and output_files[-1]['lines'] < avg_lines // 2:
        ...
        for file_info in output_files:
            print(f"   {os.path.basename(file_info['output_file'])}，共 {file_info['lines']} 行")
...
def batch_split_by_line_count(excel_file, output_excel_file):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for _, row in df.iterrows():
            ...
            futures.append(executor.submit(split_txt_by_line_count, input_file, lines_per_file))
...
if __name__ == "__main__":
    scan_root = r'D:\phpstudy_pro\WWW\wp.test\json'
