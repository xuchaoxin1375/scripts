...
class ShopifyDownloader:
    ...
    def get_sitemap_urls(self, site_url):
        ...
        if (
            site_key in self.collection_status
            and self.collection_status[site_key]["status"] == 1
        ):
        ...
        try:
            ...
            for loc in soup.find_all("loc"):
                ...
                # 匹配正确的sitemap_products格式
                # 允许: .com/sitemap_products_, .fr/sitemap_products_, .de/sitemap_products_ 等
                # 排除: .com/fr/sitemap_products_, .com/de/sitemap_products_ 等

                # 使用正则表达式匹配正确的格式
                if re.search(r"\.[a-z]{2,}\/sitemap_products_", url) and not re.search(
                    r"\.[a-z]{2,}\/[a-z]{2,}\/sitemap_products_", url
                ):
    ...
    def get_product_urls(self, sitemap_url):
        ...
        # 检查是否已采集过
        if (
            sitemap_url in self.collection_status
            and self.collection_status[sitemap_url]["status"] == 1
        ):
        ...
        try:
            ...
            for i, loc in enumerate(soup.find_all("loc"), 1):
                url = loc.text.strip()
                if "/products/" in url and not url.startswith(
                ...
                if i % 100 == 0 or i == total_locs:
                    log(
                        f"解析进度: {i}/{total_locs} ({i/total_locs*100:.1f}%)",
                        1,
                        end="\r",
                    )
    ...
    def generate_report(self):
        ...
        report_path = os.path.join(
            FILES_LIST_PATH, f"下载报告_{end_time.strftime('%Y%m%d_%H%M%S')}.txt"
        )
