...
服务器管理经常需要在服务器上修改配置文件,用户可以选择使用宝塔编辑文件,但是不够快捷键,并且对于服务器配置文件,宝塔会做额外校验,通过才能保存编辑,这初衷是好的,但并不总是我们想要的.

这里推荐一个简单易用,且符合直觉(尤其适合习惯windows的鼠标和快捷键组合的用户使用).安装和更新和使用都十分简单.

可以当做记事本使用,也可以当做高级编辑器使用.

[sinelaw/fresh: Terminal based IDE & text editor: easy, powerful and fast](https://github.com/sinelaw/fresh)

> 如果使用了本文配套的代码仓库,可以在bash中使用`install_fresh`快捷安装.
...
> 额外注意,如果要迁移服务器,如果要从备份服务器中拉取包,则需要注意包中的插件和functions.php需要全部移除并安装最新的版本;
>
> 因为插件中的代码可能发生更新,但要注意特殊站点不能应用某些插件,要使用白名单的方式安装某些插件.
...
- 备份脚本`backup_sites/get_site_pkgs.sh`
...
>
> - 如果要改其他用户的密码,把sql语句中的root替换为其他用户即可.
> - 如果是非localhost,则修改对应的登录限制字段,例如`%`
...
如果网站数量不多的时候部署网站没有遇到打不开的问题,但是随着服务器上网站的数量增多,一些不完善的模板可能会暴露出潜在的问题,例如数据库查询连接数持续积累而不释放,出现连接数占用异常的情况,甚至导致mysql客户端都连不上,例如
...
防火墙可以设置仅允许限定ip访问指定端口,以及tcp和udp流量控制,例如:

```bash
$ ufw status |grep 3306
3306/tcp                   ALLOW       120.xxx.xxx.xxx

```

如果希望任何ip可以登录,则使用`ufw allow `命令指定端口,这会添加更宽松的规则,效果如下
...
```bash
bash /www/sh/adduser_uploader.sh #将要输入密码(时uploader的密码)
```
...
OldPort=22
Port=2222 # 自行更改端口号
#(使用i命令原地修改文件并保存,注意-i选项和-n,p命令不要同时使用!);(-n和p命令配合可以打印被修改的行.)
# sed  -i -E "s/#?(Port $OldPort)/Port $Port/"  /etc/ssh/sshd_config 
#更安全的方案生成.bak备份,同时兼容行开头的空白和注释)
sed -i.bak -E "s/^[[:space:]]*#?Port $OldPort[[:space:]]*$/Port $Port/" /etc/ssh/sshd_config
...
使用 `crontab -e`选择编辑器编辑自动任务,选择默认的编辑器,或者临时修改编辑器
...
```bash
# 生成密钥对(如果未设置过的话)
ssh-keygen -t ed25519 -C "your_email or comment"
# 设置免密登录(备份服务器)
...
考虑某些网站不再需要,则不应该迁移到新服务器上.

将要移除的包名(例如网站域名),整理成一个白名单文件,推荐以LF换行风格保存.

或者直接编辑脚本`clean_bad_pkgs.sh`中的数组.

### 按需暂时取消定期自动备份设置

假设从a迁移到b,那么势必要将a中的包传输到b,对于服务器b(尤其在b不是全新的情况下,需要注意):

生产服务器上默认定期备份,例如网站包存放目录设置为:`/srv/uploads/uploader/files`,定时任务会调用`rsync`定期将该目录完整镜像到专用备份服务器中.

> 迁移过程中,如果新服务器拉取的包还没解压完就先不设置**自动备份**,检查`crontab` 里头传输到备份服务器的行暂时注释或者移除掉.
...
这个脚本功能是重要的，即便是备份服务器拥有全部备份,仍然是不可靠的,备份服务器主要是为了防止原服务器出现故障而做的解决方案.

但是备份服务器本身也可能会宕机甚至被意外破坏而不再可用,最坏情况下是不可恢复(尽管这是小概率事件),这时候求就需要从原服务器上重新导出可用(可以恢复)的备份,然后迁移到新到备份服务器上存储,只有这样才能尽可能提高可用性.

> 此外,理想情况下应该是定期更新网站备份包,因为网站的插件和`functions.php`是可能变化的(不过我们可以通过批量更新插件的方式来兜底),最重要的还是某些产品需要下架,如果使用之前的备份包,这些需要下架的产品又会重新上架(细心一点的话就是每次被下架产品的站都标记处理来,然后针对这些站做备份包的更新,但我们可以采取折中的方案,并且也是最佳实践:在迁移前仅更新数据库包,这样下架的产品就可以同步,而插件和`functions.php`保存在网站根目录中,总是会过时的,在迁移解压后,统一将所有插件更新统一更新即可,特殊插件则要使用黑(白)名单方式跳过(针对性)安装.)

关于是否支持多线程:考虑到备份大压缩包基本上都是跑满带宽,设计并行备份意义不是很大.不过后来发现,`rsync`在更新压缩包而不是全新上传的情况(传输前压缩和增量检查),速度会明显变慢.出于现象,有几个方案优化:
- 关闭`-z`选项;
- 仍然使用rsync,但是增加并行设计,让几个压缩包同时传输
- 或者直接在rsync任务启动前,计算哪些包将要备份,事先在服务器`s`上将对应的包删除
- 使用其他传输工具.

导出脚本`get_site_pkgs.sh`,可以相对灵活地将服务器上指定或者全部的网站导出到合适的目录下
...
`bash $sh/backup_sites/get_site_pkgs.sh -W whitelist.txt `,使用`--force`强制备份(覆盖已有的包)

> 更多用法示例脚本的参考`-h`输出,末尾给出了几种典型情况的用例.
...
为了描述方便,我们用服务器字母代号`a,b,s`描述要执行任务:

#### 检查可能被遗漏的站点备份

> 服务器`a`的站迁移到服务器`b`:由于我们有一台备份服务器`s`,理想情况下,每天自动将服务器`a`上新增的网站压缩包完整的备份到`s`;
>
> 但是可能有例外出现,例如某次更新脚本(包含了错误的修改)导致备份功能故障,或者备份服务器`s`恰好在备份时故障,就可能有些站没有备份到服务器`s`,为了保证备份服务器拥有`a`所有站的完整备份,此外,为了节约服务器空间,每周末都会定时清理网站压缩包,如果刚好有错过了清理前的传输(备份到`s`)
>
...
```bash
bash ./backup_sites/get_site_pkgs.sh --whitelist-site need_backup.conf  --parallel  
```
...
如果检查完所有网站后发现有些网站的没有顺利恢复,这可能是备份服务器中缺失相关的包组(sql或根目录包中的一个或全部),也可能是包损坏了.
...
>
> 如果是刚导出来的,也建议检查插件目录和functions.php文件

### 插件更新

额外注意,如果要迁移服务器,如果要从备份服务器中拉取包,则需要注意原来的中的**插件和functions.php**通常会过时,需要**全部移除并安装最新的版本**;

> 代码仓库提供的`get_site_pkgs.sh`脚本允许管理仅导出备份最新的网站数据库,而绕过网站目录的备份.
>
> 重新备份网站根目录没有必要,通常只有插件会变动,只需要重新安装插件,这会更加轻量.

因为插件中的代码可能发生更新,但要注意特殊站点不能应用某些插件,要使用**白名单的方式安装某些插件**.

### 宝塔中大批量创建站点

> 这里指的是宝塔中网站列表中添加对应的网站,方便后期管理;而不是解压网站;这个环节会覆盖伪静态!

- 使用宝塔api可以批量创建站点,但是速度并没有想象中的快速;如果线程数开太多的话,还容易报错,线程数建议不超过10;

- 另一种方案是构造宝塔约定的建站字符串,例如仓库包含专门的powershell命令来生成

  ```powershell
  Get-BatchSiteBuilderLinesFromTable -Table C:\Users\Administrator\Desktop\table-s3.conf  -Structure "Domain,User"
  ```

  这会生成形如下面的建站规则,粘贴到宝塔的批量建站中.
...
> 仓库中已经配备了对应的脚本,可以灵活地实现需求.如果api发生变更,脚本可能会失效,此时需修改脚本适配api版本
>
...
假设被迁移的某一批网站(域名)有一部分绑定到cf账号1(`cf1`),另一部分绑定cf账号2(`cf1`)

而网站从服务器`a`搬到`b`,意味着需要将cf1,cf2中的对应网站的DNS绑定的服务器`ip`地址做修改;

方便起见,我们将需要迁移到网站(列表)保存到一个**白名单文件**,每行一个域名或"url"(格式形如`domain.com`),例如保存为`white.txt`

> 指定白名单的情况下可以不用指定旧ip,仅指定新ip
>
> ```shell
>  python $pys/cf_api/cf_update_dns_ip.py --new-ip $env:DF_SERVER6 --config C:\repos\configs\deploy_configs\cf_config.json  --whitelist $desktop\domains_to_update.txt
> ```
>
> 

然后分别针对`cf1`,`cf2`(甚至更多账号也是类似的)

执行ip更改操作(命令工作目录cd到`white.txt`所在目录下)

脚本路径位于`$pys/cf_api/cf_update_dns_ip.py`

> 提供了`--config`,`--find-domain`用来批量查询多个账号中是否有哪个账号配置了指定的域名.

对于单个账号的情况:

```powershell
 python cf_update_dns_ip.py --new-ip "ip-b" --whitelist "white-list-file" --api-key "cf-api-key-x" --email "cf-x@account.com" 
```

将引号中的值替换为真实值;

对于不同账号,`--api-key "cf-api-key-x" --email "cf-x@account.com" `这里的两个值需要变换,其他参数保留不变即可.

对于多个账号的情况,建议使用配置文件提高效率:

```shell
 python cf_update_dns_ip.py --new-ip "ip-b" --whitelist "white-list-file" --config $deploy_configs/cf_config.json
```



#### cloudflare更改dns ip指向后的检查工作
...
- 检查ssl加密模式(通常设置为灵活.)
- 仔细对比同批迁移的站点中正常的网站域名的dns ip配置是否和这些有问题的域名dns ip一致.(如果ip指向错误的较多,建议使用上述脚本指定白名单的方式修改比较方便.)

> 如果其他页面404(nginx),但是可以看到robots.txt(内容不是你设置的),那么极有可能是ip指向和迁移到的新服务器ip不匹配.

> 注意,如果用户为例排查bug暂停了网站(例如宝塔中的网站暂停功能),在恢复正确ip解析后也可能遇到403错误,
>
> 这类错误,通常是ip指向正确,但是看日志的状态码可能是200这类成功状态码,但观察字节数可以发现异常.

注意修改完ip后,不一定可以立即生效,但是通常在几分钟内就可以生效.另外

- robots.txt这类特殊路径通常可以较早反映变化,这可能是cf缓存(但是观察日志可能不会立即看到相应的日志)
- 经验表明,刷新网站复查变换时,建议使用`ctrl+F5`,甚至更换浏览器或ip后重试,否则可能因为浏览器缓存问题而没有正确反映最新情况.
...
```bash
# 移动yxj的网站备份目录(从s4->s1)
merge_dir -u yxj ./s4 ./s1 deployed
```
...
> 例如我要从df_server1移动一批站到df_server3,则执行后者(df_server3)的nginx vhost配置更新
...
通常如果迁移完毕的站都能够访问了,就可以移除掉这个目录,避免占用大量空间(备份服务器中的包组依然留着);

并且,更具需要(迁移变更后),对备份服务器中的被迁移的目录做修正,比如`a`服务器上的站迁移到`b`那么`a`备份目录应该合并到`b`中

考虑到备份目录中依然保留各个人员的目录,为了结构的一致性,可以使用如下脚本进行合并更正迁移后的备份目录
