...
迁移后网站打开500系列错误,网站加载超时,504错误.

> 宝塔用户请务必检查宝塔自带的网站管理中的"**cdn源ip解析**"功能是否和本文仓库的配置冲突(`real_cdn_ip.conf`或`cf_realip.conf`在`com_com.conf`中管理.
>
> 考虑关闭宝塔的配置,防止冲突,自行配置可以提高灵活性.(尤其是使用了反向代理的用户.)

然后务必检查访问日志,观察是否有不合理的错误穿插出现,尤其是反向代理用户,要检查日志中的访问ip是否始终是同一个ip(反代服务器ip),如果是,会对nginx限流配置造成影响,导致基于ip限流策略异常.
