    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            -K | --keep-pack | --no-move-pack)
                ...
                KEEP_PACK=true
                ;;
            --rm | --remove-after-deploy)
                REMOVE_AFTER_DEPLOY=true
                ;;
            -R | --site-root-only)
                # 仅处理网站根目录(而不处理数据库)
                SITE_ROOT_SKIP="true"
...
deploy_site() {
    ...
    # 合理假设备份包所在目录(同时也是人员包存放目录):
    local pack_dir="$PACK_ROOT/$user_name"
    # 已部署网站存放目录:
    ## 静态计算的已部署网站存放目录
    ## 另一种思路是存放到解压前包所在目录的子目录deployed中,可以更好区分已部署和未部署包(但这要一个显式的意图信号,适合迁移场景.)
    local deployed_dir="$pack_dir/deployed/"

    local sql_file="$pack_dir/$domain_name.sql"
    ...
    # local extracted_domain_dir="$pack_dir/$domain_name"
    # local site_archives="$pack_dir/$domain_name.*"
    ...
    if [[ $DRY_RUN == "true" ]]; then
        # log "#[$domain_name]模拟任务耗时睡眠"
        # time_consumption=$((RANDOM % 11 + 3))
        # time_consumption=3
...
"
    ...
    # ===如果上述操作没有出错(return 1没有执行),则按需执行文件归档或者移除操作
