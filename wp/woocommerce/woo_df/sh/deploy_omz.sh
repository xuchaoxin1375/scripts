...
options:
    ...
    -zac | --install-zsh-autocomplete [true|false]
        install zsh-autocomplete plugin if true
'
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
...
echo "checking and install completion plugins..."
...
#检查修改结果
#检查配置文件是否有相应的行
cat "$zshrc_path" | grep -e zsh-syntax-highlighting -e zsh-autosuggestions -e zsh-history-substring-search -e zsh-autocomplete -e zsh-completions
