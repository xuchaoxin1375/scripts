...
show_help() {
	...
	echo "      --maxdepth <n>       find 扫描的最大深度 (默认: 3)"
...
backup_one_site() {
	...
	if [[ -n "$USER_FILTER" ]]; then
		...
		USERNAME=$(echo "$WP_DIR" | awk -F'/' '{print $(NF-3)}')
	fi
...
if [[ $PARALLEL -eq 1 ]]; then
	...
	if [[ $DRY_RUN -eq 1 ]]; then
		...
		export USER_FILTER DEST_ROOT FORCE DRY_RUN
		find "$FIND_PATH" -mindepth "$MINDEPTH" -maxdepth "$MAXDEPTH" -type d -name wordpress | \
			parallel -j "$PARALLEL_JOBS" backup_one_site
	fi
else
	# 原始串行处理模式
	while IFS= read -r WP_DIR; do
		...
		if [[ -n "$USER_FILTER" ]]; then
			...
			USERNAME_SCAN=$(echo "$WP_DIR" | awk -F'/' '{print $(NF-3)}')
		fi
