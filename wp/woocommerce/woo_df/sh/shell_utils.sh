...
echo "Loading shell_utils.sh...(version:$VERSION_UTILS)"
# 导入其他脚本
# 获取当前bash脚本的绝对路径目录（即便包含符号链接也能正确解析）
...
check_mysql() {
    ...
    local usage="
    function_version:20260529
    用法: ${FUNCNAME[0]} [-H host] [-P port] [-u user] [-p pass] [-v]
    "
    ...
    rc=$?
    # out=$(mysql "${args[@]}" --connect-timeout=3 --batch -e "SELECT 1;" 2>&1)
    # rc=$?
    if ((verbose)); then
        if ((rc == 0)); then
            echo -e "OK"
