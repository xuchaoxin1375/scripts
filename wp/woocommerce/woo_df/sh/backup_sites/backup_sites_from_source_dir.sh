...
show_help() {
	...
	echo "      --mysql-port <port>  MySQL端口"
	echo "      --mysql-user <user>  MySQL用户名"
...
check_mysql_connection() {
	# 构建mysql连接参数
	local MYSQL_ARGS=""
	[[ -n "$MYSQL_HOST" ]] && MYSQL_ARGS="$MYSQL_ARGS -h$MYSQL_HOST"
	[[ -n "$MYSQL_PORT" ]] && MYSQL_ARGS="$MYSQL_ARGS -P$MYSQL_PORT"
	...
	if [[ $DRY_RUN -eq 1 ]]; then
		if [[ -n "$MYSQL_ARGS" ]]; then
			echo "[预览] 将测试MySQL连接: mysqlshow $MYSQL_ARGS"
		else
			echo "[预览] 将测试MySQL连接: mysqlshow (使用系统默认配置)"
		fi
		return 0
	fi
	...
	if mysqlshow $MYSQL_ARGS >/dev/null 2>&1; then
		...
		echo "错误: 无法连接到MySQL服务器，请检查连接参数和服务器状态"
		return 1
	fi
}
