...
# 例如最高收费100usd,按正常比率计算对应的gbp门槛为(100/1.3)GBP,大约是76.92GBP,
# 保险起见仅允许76.00GBP,超过就不允许支付.那么把分母调高一些,让数值比76.92小一些,例如取1.31
## 2.将产品折扣力度加强,让总额尽可能低于上限.例如,将价格从gbp换算为usd
RATE_DICT_LOW = {
    "GBP": 1.25,
    "EUR": 1.1,
    "CAD": 0.65,
...
class LanguagesHotSale(EnumItRc):
    ...
    注意:
    > 对于枚举,两个枚举成员如果取值一样,则仅暴力第一个,后续的枚举成员会忽略
    > 如果有新的国家/语言添加,除了在这里添加配置,记得在woo_get_csv.py中的LanguagesHotSaleX中添加对应的语言行
