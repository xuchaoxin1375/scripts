...
def download_by_curl(
    ...
    referer: Optional[str] ="https://esd.equipment/",  # 新增参数：Referer
    timeout: int = 30,
    silent: bool = False,
    ...
    if not use_remote_name:
        if not output_path:
            raise ValueError("output_path 不能为空（除非 use_remote_name=True）")
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
    else:
        os.makedirs(output_dir, exist_ok=True)
        os.chdir(output_dir)
        parsed_url = urlparse(url)
        output_path = os.path.abspath(os.path.join(output_dir, os.path.basename(parsed_url.path)))
        print(f"使用远程文件名保存到: {output_path}")

    # 构建 curl 命令
    cmd = [
        "curl", "-f", "-L", "--retry", "3", "--retry-delay", "5", "-k",
        "-A", user_agent,
    ...
    try:
        ...
        print(f"curl 下载失败，错误码: {e.returncode}")
        return False
    finally:
        if reset_cwd:
            os.chdir(cwd)
