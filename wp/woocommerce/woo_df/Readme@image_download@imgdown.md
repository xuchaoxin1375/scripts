...
编写一个功能和文档完备,支持基本的下载统计和日志功能,还支持命令行中调用的多线程图片下载脚本(或通用模块),能够兼容各种五花八门的图片链接;用户将一个图片url列表传递给该下载器就可以进行下载;还支持接受要下载的图片的名字的指定;基于这些要求结合你的最佳实践经验推荐补充完善脚本,注意代码的可扩展性和可维护性
