...
function render_order_list_items($analysis_data, $group_by_domain, $order_sort, $status_filter, $owner_filter, $csv_path, $list_view = 'card')
{
    ...
    if ($list_view === 'table') {
        ...
        // 仅对失败组(未成功且无success_log)做前6位聚类合并；成功数量统计不受影响
        $prefix_cnt = [];
        foreach ($rows as $r0) {
    ...
    // 按排序字段先排序（后续会基于展示过滤集计算前6位聚类与合并数量）
    $sort_orders_inplace($analysis_data);

    // 卡片视图：基于展示过滤集(状态+人员)计算前6位聚类与合并数量
    $display_orders = [];
    $prefix_cnt = [];
    foreach ($analysis_data as $no => $item) {
        if (!order_matches_status_filter($item, $status_filter)) {
    ...
    <style>
        ...
        @media (min-width: 1024px) {
            ...
            .side-panel {
                width: 35%;
                position: sticky;
                top: var(--sticky-side-top, var(--sticky-filter-top, 88px));
                max-height: calc(100vh - var(--sticky-side-top, var(--sticky-filter-top, 88px)) - 12px);
                overflow: auto;
