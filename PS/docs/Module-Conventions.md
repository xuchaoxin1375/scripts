...
- 适用范围：设计型章节必须同时满足正式、严谨、清晰易懂三条；仓库根 `readme.md` 与 `PS/docs/` 用户指南参照执行（正式语言、动作三要素、术语统一为硬要求；mermaid 只在流程超三步且确有分支时配）。入口文档（根 `readme.md`、`PS/docs/README.md`、`PS/Deploy/readme.md`）只放置指引与可执行示例，不复制正文，真相源保持唯一（见 Handoff #70）；示例中的命令与章节号必须与源码、文档现状一致，改代码时同步检查。
