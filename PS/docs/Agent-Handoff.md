...
39. **文档 BOM 方案推翻（2026-09-21，用户：外部文档控不了）**：中间试过给仓库 18 个中文 `.md` 批量补 BOM（裸读当时已验），但写侧修只能覆盖自有文件，外部 uncontrolled 的 UTF-8 照样乱码，方向错，整单 revert（`f9d6e52`），conventions §8 回到 B 档例外原样。改读侧：新 `Get-Utf8Content`（Basic，B 档；.NET `ReadAllLines`/`ReadAllText` 直读，默认 UTF-8 + 自动识别 BOM，5.1/7 同行为），外部文件一次解决。另记：5.1 沙箱捕获输出本身按 GBK，验证读正确性要用字节往返（读→写文件→对字节），不能看捕获屏显——屏显乱码≠读错了。
