...
- 加新代码禁区：兼容集内禁三元 `?:`/行首管道/`Join-String`/`$PSStyle` 裸赋值/`$IsWindows` 裸分支；文件读写禁裸 `Get-Content`/`Set-Content`（5.1 默认 ANSI 页，中文必乱码），一律 `[IO.File]::ReadAllText/WriteAllText($path, UTF8)`（自动识别 BOM，双版本同行为，零跨模块依赖）；真机校验：`powershell -NoProfile -File <脚本>` 逐模块 `Import-Module` 全绿 + `init` 零失败（沙箱脚本见交接记录）。
